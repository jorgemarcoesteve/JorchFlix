...
import { FiSearch, FiMonitor, FiChevronLeft, FiChevronRight } from 'react-icons/fi';
...
export default function Series() {
  ...
  useEffect(() => {
    api.get('/media/generos?tipo=tv').then(({ data }) => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        const { data } = await api.get('/media/search', { params: { q: query, tipo: 'tv', page: pagina } });
  ...
  const filtrados = generoSeleccionado
    ? resultados.filter((s) => s.genre_ids?.includes(parseInt(generoSeleccionado)))
    : resultados;
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          <div className="flex items-center gap-3 mb-2">
            <div className="p-2.5 rounded-xl bg-gradient-to-br from-purple-500/20 to-pink-500/20">
              <FiMonitor className="text-purple-400" size={22} />
            </div>
            <div>
              <h1 className="text-3xl font-extrabold text-white tracking-tight">Series</h1>
              <p className="text-sm text-jf-muted">Busca y descubre nuevas series</p>
            </div>
          </div>
          ...
          {!cargando && query.length >= 2 && filtrados.length === 0 && (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center py-20">
              <div className="w-20 h-20 rounded-2xl bg-jf-tarjeta flex items-center justify-center mx-auto mb-6">
                <FiMonitor className="text-jf-muted/30" size={36} />
              </div>
                        ...
                        <div className="aspect-[2/3] bg-jf-hover rounded-2xl overflow-hidden relative">
                          ...
                          {poster ? (
                            ...
                            <div className="w-full h-full flex items-center justify-center">
                              <FiMonitor className="text-jf-muted/20" size={36} />
                            </div>
                          )}
